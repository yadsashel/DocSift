...
const Dashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/dashboard-stats?user_id=${userId}`);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Purge this document?")) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground pb-20 overflow-x-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10 pt-12 space-y-10">
        ...
        {/* --- Updated Header with Credits Badge --- */}
        <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 border-b border-border/50 pb-8">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto">
            ...
            <Link to="/documents" className="group flex items-center justify-center gap-3 px-8 py-4 bg-accent text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(var(--accent),0.4)]">
              Upload Vault <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
        </motion.div>

        {/* --- Rest of Dashboard (Metrics, Charts, Activity) --- */}
          ...
          <div className="glass-card p-8 rounded-3xl border border-border/40">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={chartData}>
                ...
                <Tooltip contentStyle={{ borderRadius: '16px', border: '1px solid hsl(var(--border))', background: 'hsl(var(--background))', fontSize: '10px' }} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-border/20">
                {stats.recent_activity.map((file: any) => (
                  <tr key={file.id} className="hover:bg-accent/5 transition-all group">
                    ...
                    <td className="p-6 text-right">
                      <div className="flex justify-end gap-3">
                        <button onClick={() => navigate(`/documents?id=${file.id}`)} className="p-2 hover:text-accent"><ExternalLink className="h-4 w-4" /></button>
                        <button onClick={() => handleDelete(file.id)} className="p-2 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
